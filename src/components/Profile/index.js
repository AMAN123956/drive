...
function Profile() {
	...
	const submitHandler = async(e) => {
		...
		const { data } = await axios.put(
			`${url}/api/users/update`,{name,email,password,imgurl},
			config
		);
		setloading(true)
		if(data.success){
				...
				<div className={styles.rightBox}>
					<div className='card shadow col-6 p-5'>
                        ...
                        {message && <Message variant={'success'}>{message}</Message>}    
							<div className="mb-1">
								<img src={imgurl} alt='user_img' className={styles.profileImg} />
							</div>
							<Button onClick={() => beginUpload()} className='btn btn-dark px-3'>Edit</Button>
							<Form.Group>
								<Form.Control type='text' value={imgurl} className='d-none' /> 
							</Form.Group>
						    <Form.Group className="mb-3" controlId="formBasicEmail">
								<Form.Label>Username</Form.Label>
								<Form.Control type="username"
									value={name} onChange={usernameHandler}
									placeholder="Update Username" required='true' />
						    </Form.Group>
							<Form.Group className="mb-3" controlId="formBasicEmail">
							...
							{
                                loading ? <Loader1></Loader1> :<Button variant="primary" type="submit" onClick={submitHandler} disabled={loading}>Update</Button>
                            }
						</Form>
					</div>
				</div>
